...
def get_points_by_team_and_season(db: Session, team: str, season: str):
    return db.query(models.PointEvent).filter(models.PointEvent.team == team, models.PointEvent.season == season).all()

def get_points_by_player_and_season(db: Session, player: str, season: str):
    return db.query(models.PointEvent).filter(models.PointEvent.player == player, models.PointEvent.season == season).all()